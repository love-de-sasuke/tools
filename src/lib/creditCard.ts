...
export function calculateCreditCardPayoff(
  ...
  const extraBudget = parsed.extraMonthlyBudget
    ? toDecimal(parsed.extraMonthlyBudget, { maxDp: 2 })
    : ZERO;

  if (extraBudget.lt(0)) {
    throw new Error("Extra budget cannot be negative");
  }
  ...
  const debts = parsed.debts.map((d) => {
    ...
    if (
      ...
      throw new Error("Invalid debt inputs");
    }
  ...
  const order = (items: typeof debts) => {
    if (parsed.strategy === "snowball") {
      return [...items].sort((a, b) =>
        a.balance.comparedTo(b.balance)
      );
    }
    return [...items].sort((a, b) =>
      b.apr.comparedTo(a.apr)
    );
  };
  ...
  if (month >= maxMonths) {
    throw new Error(
      "Payoff did not converge; increase payments or reduce debt list."
    );
  }
...
export function calculateCreditCardPayoffSeries(
  raw: unknown,
  opts?: { sampleEveryMonths?: number }
) {
  const parsed = PayoffSchema.parse(raw);
  ...
  const totalBalance = () =>
    working.reduce(
      (acc, d) => acc.add(Decimal.max(d.balance, ZERO)),
      ZERO
    );
  ...
  while (month < maxMonths) {
    ...
    for (const d of active) {
      const monthlyRate = d.apr.div(12);
      d.balance = d.balance.add(
        d.balance.mul(monthlyRate)
      );
    }
