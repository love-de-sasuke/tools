...
export function calculateLoan(raw: unknown): LoanResult {
  ...
  if (principal.lte(0)) throw new Error("Principal must be greater than 0");

  const aprPercent = toDecimal(parsed.annualInterestRatePercent, { maxDp: 6 });
  if (aprPercent.lt(0) || aprPercent.gt(100)) throw new Error("APR must be between 0 and 100");

  const years = toDecimal(parsed.termYears, { maxDp: 6 });
  if (years.lte(0) || years.gt(100)) throw new Error("Term must be between 0 and 100 years");

  const extra = parsed.extraMonthlyPayment ? toDecimal(parsed.extraMonthlyPayment, { maxDp: 2 }) : new Decimal(0);
  if (extra.lt(0)) throw new Error("Extra payment cannot be negative");

  const startDate = parsed.startDateISO ? new Date(`${parsed.startDateISO}T00:00:00.000Z`) : new Date();
  ...
  if (months.lte(0)) throw new Error("Term must be at least 1 month");
  ...
  const baseMonthlyPayment = (() => {
    ...
    return principal.mul(monthlyRate.mul(pow)).div(pow.sub(1));
  ...
  for (let i = 1; i <= months.toNumber(); i++) {
    ...
    const interest = monthlyRate.eq(0) ? new Decimal(0) : balance.mul(monthlyRate);
    ...
    const payment = Decimal.min(rawPayment, paymentCap).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);

    const interestPaid = Decimal.min(interest, payment).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const principalPaid = payment.sub(interestPaid).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);

    balance = balance.sub(principalPaid).toDecimalPlaces(10, Decimal.ROUND_HALF_UP);
    if (balance.abs().lt(new Decimal("0.005"))) balance = new Decimal(0);
    ...
    schedule.push({
      ...
      extraPayment: moneyUSD(Decimal.min(extraPayment, payment.sub(scheduledPayment).max(0))),
      balance: moneyUSD(balance.max(0))
    });
  }

  const payoffDateISO = schedule.length ? schedule[schedule.length - 1].dateISO : toISODateUTC(startDate);
